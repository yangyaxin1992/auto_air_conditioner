...
class JudgeLog():
    ...
    # 获取开关、模式、风速、温度响应resp
    def get_set_resp(self, key, log_name, sno,endpoint):
        ...
        try:
            with open(self.path + log_name, encoding='utf-8') as obj:
                ...
                symbol = '.*\n'
                regex_onoff = r'LOG_DEBUG ({.*\n.*"msgType"' + '.*' + '"DEVICE_CONTROL_RESP"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3 + '.*"sno"' + '.*' + sno + '"' + symbol * 2 + '.*"command"' + '.*' + '"set_powerstate_resp"' + symbol * 11 + '})'
                regex_model = r'LOG_DEBUG ({.*\n.*"msgType"' + '.*' + '"DEVICE_CONTROL_RESP"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3 + '.*"sno"' + '.*' + sno + '"' + symbol * 2 + '.*"command"' + '.*' + '"set_model_resp"' + symbol * 11 + '})'
                regex_windspeed = r'LOG_DEBUG ({.*\n.*"msgType"' + '.*' + '"DEVICE_CONTROL_RESP"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3  + '.*"sno"' + '.*' + sno + '"' + symbol * 2 + '.*"command"' + '.*' + '"set_windspeed_resp"' + symbol * 11 + '})'
                regex_temp = r'LOG_DEBUG ({.*\n.*"msgType"' + '.*' + '"DEVICE_CONTROL_RESP"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3  + '.*"sno"' + '.*' + sno + '"' + symbol * 2 + '.*"command"' + '.*' + '"set_temperature_resp"' + symbol * 11 + '})'
                if key == 'powerstate':
                    # print("reps:",regex_onoff)
    ...
    def judge_set_resp(self, key, log_name, sno,endpoint):
        code_data = self.get_set_resp(key, log_name, sno,endpoint)
        if code_data == False:
            return False
        else:
    ...
    def judge_device_notify(self, key, log_name, sno, state,endpoint):
        ...
        if code_data == False:
            ...
            sn = code_data['data'][2]['v'] # 3.2.0版本这里的时间戳改成了时间，需要做转换
            sn = sn.split('.')  #去掉小数点后的毫秒，数字占位符太长转换有问题
            s_t = time.strptime(sn[0],"%Y-%m-%d %H:%M:%S")
            callbackTime = int(time.mktime(s_t))
            sno_1 = str(callbackTime)
            ...
            try:
                with open(self.path + log_name, encoding='utf-8') as obj:
                    ...
                    symbol = '.*\n'
                    regex_onoff_change = r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_powerstate_change"' + symbol * 6 + '})'
                    regex_model_change = r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_model_change"' + symbol * 6 + '})'
                    regex_windspeed_change = r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3+ '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_windspeed_change"' + symbol * 6 + '})'
                    regex_temp_change = r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 1+'.*"devId'+'.*'+ endpoint + symbol * 3 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_temperature_change"' + symbol * 6 + '})'
                    if key == 'powerstate':
                ...
                if len(r) == 0:
                    ...
                    while i < 5:
                        ...
                        if key == 'powerstate':
                            r = re.findall(
                                r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 4 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_powerstate_change"' + symbol * 6 + '})',
                                log)
                        elif key == 'model':
                            r = re.findall(
                                r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 4 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_model_change"' + symbol * 6 + '})',
                                log)
                        elif key == 'windspeed':
                            r = re.findall(
                                r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 4 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_windspeed_change"' + symbol * 6 + '})',
                                log)
                        elif key == 'temperature':
                            r = re.findall(
                                r'LOG_DEBUG ({\n.*"msgType"' + '.*' + '"DEVICE_NOTIFY"' + symbol * 4 + '.*"sno"' + '.*' + sno_1 + '.*"' + symbol * 2 + '.*"command"' + '.*' + '"on_temperature_change"' + symbol * 6 + '})',
                                log)
                        # print("更新sno后查询出的NOTIFY数据：",r)
